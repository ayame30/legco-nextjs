...
const Bill = ({ id }) => {
  const { data, data: { bill, billDescription, billReading } = {}, loading, error } = useQuery(BILL_QUERY, {
  ...
  return (
    <div>
      <Head>
        <title>議案 - {billData.title}</title>
        <meta name="description" content={billData.description} />
      </Head>
      <div className='hero'>
          <BillPage {...billData} />
      </div>
